...
class F1utter extends StatefulWidget {
  ...
  _F1utterState createState() {
    SharedPreferences.getInstance().then((prefs) {
      ...
      GlobalData.appBrightness.value =
          br == "Brightness.dark" ? Brightness.dark : Brightness.light;
      if (primary != null)
        GlobalData.primaryColor.value =
            Colors.primaries.where((color) => color.value == primary).first;
      if (accent != null)
        GlobalData.accentColor.value =
            Colors.accents.where((color) => color.value == accent).first;
    });
