...
class ApiHelper {
  static final Uri _seasonUri = new Uri.https(
      "ergast.com", "/api/f1/" + new DateTime.now().year.toString() + ".json");
  ...
  static Future makeRequest(Uri uri) async {
  ...
  static Future<Map> getRaceResultsByRound(
      {String year = "2018", String round}) {
    return makeRequest(raceResultUri(year: year, round: round)).then((res) {
      ...
      for (var result in results) {
        raceResultList.add(new RaceResult(
          number: result["number"],
          position: result["position"],
          points: result["points"],
          driver: DriverList.driver(driverEntry: result["Driver"]),
          constructor: ConstructorList.constructor(
              constructorEntry: result["Constructor"]),
          grid: result["grid"],
          laps: result["laps"],
          status: result["status"],
          time:
              result["Time"] != null ? result["Time"]["time"] : "Not Finished",
