...
class RaceResultsPageState extends State<RaceResultPage> {
  ...
  void initState() {
    ...
    ApiHelper.getRaceResultsByRound(
            year: widget.raceData.raceTime.year.toString(),
            round: widget.raceData.round)
        .then((res) {
  ...
  void _fillRows() {
    ...
    for (var row in res) {
      rows.add(new DataRow(cells: [
        ...
        new DataCell(
            new Text(row.driver.givenName + " " + row.driver.familyName)),
  ...
  void _snackBar(msg) {
    scaffoldKey.currentState
        .showSnackBar(new SnackBar(content: new Text(msg.toString())));
  }
